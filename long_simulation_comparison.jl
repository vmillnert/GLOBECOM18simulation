...
# decide which of the controllers we should run
DAS   = true
DOA   = false
AC    = false

DAS_low = 0.7 # lower threshold  => scale down if below
DAS_high = 0.9 # upper threshold => scale up if above
...
deadline_low  = 0.5  # time-units (in seconds)
deadline_high = 1.0  # time-units
...
for i in 1:N
    ...
    for v in 1:n
        ...
        # the next part should only be done if the admission
        # controller (AC) should be used...
        if AC && i + D[v] < N
        ...
        if i > 1
            if DAS #&& i > Delta[v] # m_ref according to automatic autoscaling
                if  ue[v,i-1] > DAS_high
                    mref_nofilt[v,i] = m[v,i] + 1
                elseif ue[v,i-1] < DAS_low
                    mref_nofilt[v,i] = m[v,i] - 1
                else
                    mref_nofilt[v,i] = m[v,i]
                end
                if i > filt
                    mref[v,i] = round(Int64, mean(mref_nofilt[v,i-filt:i]))
                else
                    mref[v,i] = round(Int64, mref_nofilt[v,i-1]*0.8 + mref_nofilt[v,i]*0.2)
                end

            elseif DOA
                mref[v,i] = round(Int64, r[v,i]*DOA_ratio/ns[v])
            end
        end
...
println("average utility: $(mean(U))")
println("average efficiency: $(mean(Ue))")
println("average availability: $(mean(Ua))")
